...
### 1. `genkit-coach-flow` — `src/ai/flows/genkit-coach-flow.ts`
...
### 2. `coach-api-route` — `src/app/api/chat/coach/route.ts`
...
### 3. `search-worker-client` — `src/lib/search/search-worker-client.ts`
...
| 2026-07-11 | `genkit-coach-flow` (stub)                   | `src/ai/flows/genkit-coach-flow.ts`      | **Promoted to real implementation.** Now delegates to `streamCoachResponse` (issue #1071). See Detail §1.                                                                                                                                                                      |
| 2026-07-11 | `coach-api-route` (stub)                     | `src/app/api/chat/coach/route.ts`        | **Promoted to real implementation.** Now backed by `streamCoachResponse` + `eventToSse` (issue #1077). See Detail §2.                                                                                                                                                          |
| 2026-03-20 | `ai-coach-conversational` ("To finish" note) | `src/components/ai-coach/chat-panel.tsx` | **Mounted on `/deck-coach`.** Conversational AI Coach shipped in v1.7 (Phases 27-30). The mounted panel is `DeckCoachChatPanel` (`src/components/chat/chat-panel.tsx`). **File deleted 2026-09-16** by the orphan cleanup (PR #1846); the Detail section was retired with it.                                                                                         |
| 2024-XX-XX | `search-worker-client` (stub)                | `src/lib/search/search-worker-client.ts` | **Promoted to real implementation.** Real Orama Web Worker ships via `src/lib/search/search.worker.ts` (issue #1389). See Detail §3.                                                                                                                                           |
