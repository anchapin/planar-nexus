...
> **⚠ DEPRECATED (post-v1.8)**: The AI_PROXY endpoint schema in §2.2 has known inaccuracies — `provider` is `z.string().optional()` in the current route schema (not a required union), and `endpoint` is required (not optional). Prefer [AI_PROVIDER_SETUP.md](./AI_PROVIDER_SETUP.md) for provider wire IDs and client-side usage patterns. AI_PROXY endpoint reference in this doc may be stale; verify against `src/app/api/ai-proxy/route.ts`.
...
| ---------- | --------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------- |
| POST       | `/api/auth/session`               | Set session cookie (`{ userId }` in body)                                                                                                                                                                                                              | [`auth/session/route.ts`](../src/app/api/auth/session/route.ts)                             |
| DELETE     | `/api/auth/session`               | Clear session cookie                                                                                                                                                                                                                                   | [`auth/session/route.ts`](../src/app/api/auth/session/route.ts)                             |
...
> **Heads up:** the client-side wrapper surfaces below are **not** Next.js
> Server Actions — none of the modules carry a `"use server"` directive.
> They used to live in a file misnamed `src/app/actions.ts`; that file was
> renamed/relocated in issue #1592. The AI wrappers now live in
> [`src/lib/ai-client.ts`](../src/lib/ai-client.ts), card search in
> [`src/lib/card-database.ts`](../src/lib/card-database.ts), and deck
> persistence in [`src/lib/deck-storage.ts`](../src/lib/deck-storage.ts).
> They call the AI flows in [`src/ai/flows/`](../src/ai/flows) (and the
> routes in [Section 2](#2-api-routes)) from the browser. Real persistence
> is client-side via IndexedDB/Dexie. The signatures are kept here as the
...
```typescript
// src/lib/card-database.ts (client-side — no "use server" directive; see §7 callout)
...
```typescript
// src/lib/deck-storage.ts (client-side — no "use server" directive; see §7 callout)
...
```typescript
// src/lib/ai-client.ts (client-side wrappers — no "use server" directive; see §7 callout)
...
const DEFAULT_RATE_LIMIT_MAX = parseInt(
  process.env.NEXT_PUBLIC_AI_RATE_LIMIT_MAX || "10",
  10,
);
const DEFAULT_RATE_LIMIT_WINDOW_MS = parseInt(
  process.env.NEXT_PUBLIC_AI_RATE_LIMIT_WINDOW_MS || "60000",
  10,
);
const DEFAULT_DEBOUNCE_MS = parseInt(
  process.env.NEXT_PUBLIC_AI_DEBOUNCE_MS || "500",
  10,
);
```
