...
| Inbound trust pipeline (per-link rate limit → safe parse → shape/envelope gate → `seq` anti-replay → role allowlist → host legality) | `src/lib/p2p-inbound-pipeline.ts`                                                       | Issue #1791: the ordered validation pipeline shared by BOTH the 1:1 and mesh connections. The two connections configure it (limits, keys, roles, host gate) and own no inline pipeline logic of their own. Role allowlist policy via `src/lib/peer-role.ts`; anti-replay via `src/lib/anti-replay-tracker.ts`. |
| Connection / message framing, roles, anti-replay                                                                                     | `src/lib/p2p-game-connection.ts`, `src/lib/mesh-game-connection.ts`                     | Trust boundary ENTRY POINTS (delegate inbound validation to `src/lib/p2p-inbound-pipeline.ts`); role gating via `src/lib/peer-role.ts`.                                                                                                                                                                        |
