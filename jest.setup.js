    ...
    return this.cards.filter(
      (card) =>
        ...
        card.subtypes?.some((subtype) =>
          subtype.toLowerCase().includes(lowerQuery),
        ),
    );
  },
...
/**
 * Seeds the database with M21 test cards for draft/sealed testing.
 * Call this in beforeAll or beforeEach for tests requiring set m21.
 */
global.seedM21Cards = async () => {
  try {
    const { m21TestCards } = require("./src/lib/__fixtures__/test-cards");

    mockCardDatabase.cards = [...m21TestCards];
    mockCardDatabase.isInitialized = true;

    // Also add cards to IndexedDB
    const {
      addCards,
      initializeCardDatabase,
    } = require("./src/lib/card-database");
    await initializeCardDatabase();
    await addCards(m21TestCards);

    return m21TestCards;
  } catch (err) {
    console.warn("Could not seed M21 cards:", err.message);
    return [];
  }
};
