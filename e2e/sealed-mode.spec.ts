...
test.describe('Sealed Mode - Set Browser', () => {
  ...
  test('SET-01: should navigate to set browser', async ({ page }) => {
    ...
    await expect(page.locator('h1, h2').filter({ hasText: /set|Set/i })).toBeVisible();
  });
  ...
  test('SET-01: should display MTG sets', async ({ page }) => {
    ...
    const setsContainer = page.locator('[class*="grid"], [class*="list"], [data-testid*="set"]').first();
  ...
  test('SET-02: should allow selecting a set', async ({ page }) => {
    ...
    const firstSet = page.locator('[class*="card"], [class*="set"]').filter({ hasText: /[A-Z]{2,}/i }).first();
  ...
  test('SET-03: should show set details before confirming', async ({ page }) => {
    await page.waitForTimeout(2000);
    
    // Click a set to open details
    const setsWithCards = page.locator('[class*="card"], [class*="set"]').first();
    ...
    if (await setsWithCards.isVisible({ timeout: 5000 })) {
      ...
      const startSealedButton = page.locator('button').filter({ hasText: /sealed|Start/i }).first();
  ...
  test('SET-02: should navigate to sealed page on Start Sealed', async ({ page }) => {
    await page.waitForTimeout(2000);
    ...
    if (await firstSet.isVisible({ timeout: 5000 })) {
      ...
      const startButton = page.locator('button').filter({ hasText: /Start.*Sealed|Start.*sealed/i }).first();
...
test.describe('Sealed Mode - Pool Display', () => {
  test('SEAL-01: should create sealed session', async ({ page }) => {
    ...
    if (await firstSet.isVisible({ timeout: 5000 })) {
      ...
      if (await startButton.isVisible({ timeout: 2000 })) {
        ...
        const poolSection = page.locator('[class*="pool"], [class*="cards"], [data-testid*="pool"]').first();
  ...
  test('SEAL-02: should display sealed pool with cards', async ({ page }) => {
    ...
    const cardsContainer = page.locator('[class*="grid"], [class*="card"]').first();
  ...
  test('SEAL-02: should show all cards (no face-down packs)', async ({ page }) => {
...
test.describe('Sealed Mode - Pool Filtering', () => {
  test('SEAL-03: should filter by color', async ({ page }) => {
    ...
    const colorFilters = page.locator('button').filter({ hasText: /^[WUBRG]$/i });
  ...
  test('SEAL-03: should filter by type', async ({ page }) => {
    ...
    const typeFilters = page.locator('button, [class*="filter"]').filter({ hasText: /creature|instant|sorcery|artifact|enchantment/i });
  ...
  test('SEAL-03: should filter by CMC/mana cost', async ({ page }) => {
    ...
    const cmcFilter = page.locator('[class*="cmc"], [class*="mana"], input[type="range"]').first();
...
test.describe('Sealed Mode - Limited Deck Builder', () => {
  test('LBld-01: should navigate to limited deck builder', async ({ page }) => {
    ...
    const buildDeckButton = page.locator('button').filter({ hasText: /Build.*Deck|Deck.*Build|Build Deck/i }).first();
  ...
  test('LBld-01: should show pool cards in limited deck builder', async ({ page }) => {
    await page.goto('/limited-deck-builder');
    await page.waitForTimeout(2000);
    
    // Should show pool section
    const poolSection = page.locator('[class*="pool"], [class*="source"]').first();
  ...
  test('LBld-03: should show 40-card minimum validation', async ({ page }) => {
    ...
    const validationMsg = page.locator('text=/minimum|40 cards|valid/i').first();
  ...
  test('LBld-04: should enforce 4-copy limit', async ({ page }) => {
    ...
    // Find a card in pool
    const poolCards = page.locator('[class*="pool"] [class*="card"], [class*="source"] [class*="card"]');
    
    if (await poolCards.first().isVisible({ timeout: 3000 })) {
      // Add the same card multiple times (if add button exists)
      const addButton = poolCards.first().locator('button').filter({ hasText: /\\+|add|Add/i }).first();
  ...
  test('LBld-03: should show validation error for deck below 40 cards', async ({ page }) => {
    await page.goto('/limited-deck-builder');
    await page.waitForTimeout(2000);
    ...
    const actionButton = page.locator('button').filter({ hasText: /Save|Save Deck|Play/i }).first();
...
test.describe('Sealed Mode - Pool Isolation', () => {
  test('ISOL-01: pool cards should not appear in regular deck builder', async ({ page }) => {
    ...
    if (await searchInput.isVisible({ timeout: 2000 })) {
      ...
      const poolIndicator = page.locator('[class*="pool"], text=/pool/i').first();
  ...
  test('ISOL-03: should use separate IndexedDB store for sessions', async ({ page }) => {
  ...
  test('should load existing session by ID', async ({ page }) => {
    ...
    if (await firstSet.isVisible({ timeout: 5000 })) {
      ...
      if (await startButton.isVisible({ timeout: 2000 })) {
        ...
        const param = sessionMatch ? `session=${sessionMatch[1]}` : (setMatch ? `set=${setMatch[1]}` : '');
...
test.describe('Sealed Mode - Navigation Flow', () => {
  test('should navigate Set Browser -> Sealed -> Limited Deck Builder', async ({ page }) => {
    ...
    if (await firstSet.isVisible({ timeout: 5000 })) {
      ...
      if (await startButton.isVisible({ timeout: 2000 })) {
        ...
        if (await buildDeckButton.isVisible({ timeout: 2000 })) {
          ...
          const viewPoolButton = page.locator('button').filter({ hasText: /View.*Pool|Pool/i }).first();
