...
test.describe('AI Streaming & Tools', () => {
  ...
  test('should support streaming responses in AI Coach chat', async ({ page }) => {
  ...
  test('should fallback to heuristic mode when offline', async ({ page }) => {
    ...
    // Trigger deck review
    await page.goto('/deck-coach');
    await page.fill('textarea[placeholder*="1 Sol Ring"]', '1 Black Lotus\n1 Mox Ruby');
  ...
  test('should handle tool calling (searchCards) correctly', async ({ page }) => {
    ...
    expect(response).toContain('9:{"toolCallId":"call_123","toolName":"searchCards"');
    expect(response).toContain('a:{"toolCallId":"call_123","toolName":"searchCards"');
