...
test.describe("Phase 20: Advanced Optimization", () => {
  ...
  test("mana curve tab shows analysis when deck has cards", async ({
    page,
  }) => {
    await page.goto("/deck-builder");
    ...
    const hasAverageCMC = await page
      .getByText(/average cmc/i)
      .isVisible()
      .catch(() => false);
    const hasManaCurve = await page
  ...
  test("deck coach mana curve tab exists", async ({ page }) => {
    ...
    await expect(
      page.getByRole("button", { name: /review my deck/i }),
    ).toBeVisible();
  });
