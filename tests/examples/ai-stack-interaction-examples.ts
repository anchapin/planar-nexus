...
} from "@/ai/stack-interaction-ai";
import { GameState, PlayerState } from "@/ai/game-state-evaluator";
...
function toGameStateStackItem(action: StackAction): {
  ...
  return {
    ...
    targets: action.targets
      ?.map((t) => t.playerId || t.permanentId || t.cardId || "")
      .filter(Boolean),
  };
}
...
export function example1BasicCounterspell(): void {
  ...
  console.log(
    `Opponent casts: ${stackAction.name} (${stackAction.manaValue} mana)`,
  );
...
export function example6HoldPriority(): void {
  ...
  console.log(
    `We also have: ${instantBuff.name} (${instantBuff.manaValue} mana)`,
  );
...
export function example8CardAdvantage(): void {
  ...
  console.log(
    `Opponent's hand: ${gameState.players["player2"].hand.length} cards`,
  );
