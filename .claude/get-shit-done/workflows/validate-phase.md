...
```bash
AUDITOR_MODEL=$(node "/home/alex/Projects/ResumeAI/.claude/get-shit-done/bin/gsd-tools.cjs" resolve-model gsd-nyquist-auditor --raw)
NYQUIST_CFG=$(node "/home/alex/Projects/ResumeAI/.claude/get-shit-done/bin/gsd-tools.cjs" config get workflow.nyquist_validation --raw)
```
...
find . \( -name "*.test.*" -o -name "*.spec.*" -o -name "test_*" \) -not -path "*/node_modules/*" -not -path "*/venv/*" 2>/dev/null | head -40
```
...
```
Task(
  prompt="Read /home/alex/Projects/ResumeAI/.claude/agents/gsd-nyquist-auditor.md for instructions.\n\n" +
...
**State B (create):**
1. Read template from `/home/alex/Projects/ResumeAI/.claude/get-shit-done/templates/VALIDATION.md`
...
node "/home/alex/Projects/ResumeAI/.claude/get-shit-done/bin/gsd-tools.cjs" commit "docs(phase-${PHASE}): add/update validation strategy"
```
