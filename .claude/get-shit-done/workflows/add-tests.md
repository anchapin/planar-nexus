...
find . -type d \( -name "*test*" -o -name "*spec*" -o -name "*__tests__*" \) -not -path "*/node_modules/*" -not -path "*/venv/*" 2>/dev/null | head -20
# Find existing test files for convention matching
find . -type f \( -name "*.test.*" -o -name "*.spec.*" -o -name "*Tests.fs" -o -name "*Test.fs" \) -not -path "*/node_modules/*" -not -path "*/venv/*" 2>/dev/null | head -20
...
```bash
node "/home/alex/Projects/ResumeAI/.claude/get-shit-done/bin/gsd-tools.cjs" state-snapshot
```
