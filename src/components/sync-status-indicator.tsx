  ...
  if (compact) {
    return (
      <div className="flex items-center gap-2">
        <div
          className={`flex items-center gap-1 px-2 py-1 rounded-md ${syncState.bgColor}`}
        >
          <Icon className={`h-4 w-4 ${syncState.color}`} />
          <span className={`text-sm ${syncState.color}`}>
            {syncState.label}
          </span>
        </div>
        {desyncAlert && (
      ...
      <Card className="w-full">
        ...
        <CardContent>
            ...
            <div className="flex items-center gap-2">
              ...
              <div>
                ...
                <p className="font-medium">
                  {(status.statistics.matchRate * 100).toFixed(1)}%
                </p>
              </div>
            </div>
            <div className="flex items-center gap-2">
              ...
              <AlertDescription>
                <p className="text-sm mt-1">
                  ...
                  <code className="bg-muted px-1 rounded">
                    {desyncAlert.peerId?.slice(0, 8)}...
                  </code>{" "}
                  has a different game state.
                </p>
            ...
            <div className="space-y-4 pt-4 border-t">
              ...
              <div>
                ...
                <div className="space-y-1">
                  {Array.from(status.peerHashes.entries()).map(
                    ([peerId, hash]) => (
                      <div
                        key={peerId}
              ...
              <div>
                ...
                <div className="grid grid-cols-2 gap-2 text-sm">
                  <div>
                    ...
                    <span className="ml-2">
                      {status.statistics.totalChecks}
                    </span>
                  </div>
                  <div>
                    <span className="text-muted-foreground">Mismatches:</span>
                    <span className="ml-2">
                      {status.statistics.mismatchCount}
                    </span>
                  </div>
                  <div>
                    <span className="text-muted-foreground">
          ...
          <div className="flex gap-2 mt-4">
            ...
            >
              <RefreshCw
                className={`h-4 w-4 mr-2 ${isResolving ? "animate-spin" : ""}`}
              />
              Check Sync
            </Button>
            {desyncAlert && (
      ...
      >
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              A desynchronization has been detected between your game state and
              a peer's state.
            </DialogDescription>
          </DialogHeader>
          ...
          <div className="space-y-4">
            <Alert>
              ...
              <AlertDescription>
                {desyncAlert?.description ||
                  "Your game state differs from your opponent's state. This can happen due to network issues or timing differences."}
              </AlertDescription>
            </Alert>
