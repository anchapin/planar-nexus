...
export function IndexingProgress() {
  ...
  if (status === "error") {
    return (
      <Alert variant="destructive" className="my-4">
        ...
        <AlertDescription>
          {error?.message ||
            "An unknown error occurred while indexing cards for local search."}
        </AlertDescription>
      </Alert>
    );
  }
  ...
  if (status === "completed") {
    return (
      <Alert
        variant="default"
        className="my-4 border-green-500/50 bg-green-500/10 text-green-700 dark:text-green-400"
      >
  ...
  return (
    <div className="my-4 space-y-3 rounded-lg border bg-card p-4 text-card-foreground shadow-xs">
