...
export type SpellColor =
  "blue" | "red" | "green" | "black" | "white" | "colorless" | "multicolor";
...
export function SpellCastEffect({
  event,
  onComplete,
  className,
}: SpellEffectProps) {
  const [phase, setPhase] = useState<"start" | "charging" | "burst" | "fade">(
    "start",
  );
...
export function CounterSpellEffect({
  event,
  onComplete,
  className,
}: SpellEffectProps) {
  const [phase, setPhase] = useState<"appear" | "counter" | "shatter" | "fade">(
    "appear",
  );
    ...
    >
      ...
      {phase === "shatter" &&
        particles.map((p) => (
          <div
            key={p.id}
            className="absolute w-3 h-3 bg-red-400 rounded-xs"
            style={{
              left: "50%",
              top: "50%",
...
// Card draw effect
export function CardDrawEffect({
  event,
  onComplete,
  className,
}: SpellEffectProps) {
...
// Discard/mill effect
export function DiscardMillEffect({
  event,
  onComplete,
  className,
}: SpellEffectProps) {
...
export function TokenEffect({
  event,
  onComplete,
  className,
}: SpellEffectProps) {
  const [phase, setPhase] = useState<"spawn" | "grow" | "settle" | "done">(
    "spawn",
  );
...
export function TransformEffect({
  ...
  const [phase, setPhase] = useState<"start" | "flip" | "reveal" | "done">(
    "start",
  );
  const [rotation, setRotation] = useState(0);
...
export function SacrificeEffect({
  ...
  useEffect(() => {
    const crumbleTimer = setTimeout(() => {
      setPhase("crumble");
      setParticles(
        Array.from({ length: 12 }).map((_, i) => ({ id: i, delay: i * 50 })),
      );
    }, 100);
      ...
      >
        ...
        {phase === "crumble" &&
          particles.map((p) => (
            <div
              key={p.id}
              className="absolute w-2 h-2 bg-orange-400 rounded-xs"
              style={{
...
// Exile effect
export function ExileEffect({
  event,
  onComplete,
  className,
}: SpellEffectProps) {
  const [phase, setPhase] = useState<"start" | "warp" | "banish" | "done">(
    "start",
  );
...
export function ScryEffect({ event, onComplete, className }: SpellEffectProps) {
  ...
  const [topCards, setTopCards] = useState<
    Array<{ id: number; keep: boolean | null }>
  >([]);

  useEffect(() => {
    const amount = event.amount || 1;
    setTopCards(
      Array.from({ length: amount }).map((_, i) => ({ id: i, keep: null })),
    );
...
export function SpellEffects({
  ...
  return (
    ...
    >
      {events.map((event) => (
        <SpellEffect key={event.id} event={event} onComplete={handleComplete} />
      ))}
    </div>
  );
}
...
export function useSpellEvents({
  maxEvents = 5,
}: UseSpellEventsOptions = {}): UseSpellEventsReturn {
  ...
  const triggerSpell = useCallback(
    (
      type: SpellEffectType,
      color?: SpellColor,
      cardType?: CardType,
  ...
  const triggerCast = useCallback(
    (cardName: string, color: SpellColor, cardType: CardType) => {
  ...
  const triggerToken = useCallback(
    (cardName: string, amount: number) => {
  ...
  const triggerSacrifice = useCallback(
    (cardName: string) => {
  ...
  const triggerExile = useCallback(
    (cardName: string) => {
