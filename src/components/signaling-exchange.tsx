...
export interface SignalingExchangeProps {
  ...
  /** Mode: 'host' or 'client' */
  mode: "host" | "client";
  /** Actionable diagnostic from the last connection failure (issue #926) */
      ...
      <CardContent className="space-y-6">
        ...
        {step === "failed" && (
          <div className="text-center py-8">
            ...
            {connectionFailureReason ? (
              <div className="mt-3 text-sm text-muted-foreground text-left bg-muted/50 border rounded-lg p-3 space-y-2">
                <p>{connectionFailureReason.reason}</p>
                <p className="font-medium text-foreground">
                  {connectionFailureReason.remediation}
                </p>
              </div>
            ) : (
              <p className="text-sm text-muted-foreground mt-2">
