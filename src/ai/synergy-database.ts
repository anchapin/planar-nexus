 ...
 */
export type SynergyType = 'keyword' | 'tribal' | 'mechanic' | 'mana' | 'combo' | 'theme';
...
export const SYNERGY_DATABASE: SynergyEntry[] = [
  // === KEYWORD SYNERGIES (4) ===
  {
    ...
    score: 8,
    description: 'Creatures with flying and deathtouch are nearly unblockable and trade favorably',
    example: 'Necrotic Dragon, Dragonlord Silumgar',
    minimumCards: 4,
    missingSuggestions: [
      {
        missing: 'Evasion enhancement',
        description: 'Your flying deathtouch creatures could benefit from unblockable or shadow',
        ...
        condition: (deck) => {
          ...
          const unblockable = countKeyword(deck, 'unblockable') + countKeyword(deck, 'shadow');
  ...
  },
  {
    ...
    score: 7,
    description: 'First and double strike creatures win combat and deal damage safely',
    example: 'Aurelia, the Warleader + double strike creatures',
    minimumCards: 4,
    missingSuggestions: [
      {
        missing: 'Pump spells',
        description: 'Your strike creatures could benefit from pump spells to maximize damage',
        suggestion: ['Mutagenic Growth', 'Titanic Growth', 'Berserk'],
        impact: 'medium',
        condition: (deck) => {
          const strike = countKeyword(deck, 'first strike') + countKeyword(deck, 'double strike');
          const pump = countPattern(deck, ['pump', 'growth', 'might', 'strength']);
    ...
    missingSuggestions: [
      {
        ...
        description: 'You have lifelink but no cards that benefit from life gain',
        suggestion: ['Ajani\'s Pridemate', 'Soul Warden', 'Celestial Mantle'],
        impact: 'high',
        condition: (deck) => {
          const lifelink = countKeyword(deck, 'lifelink');
          const payoff = countPattern(deck, ['lifelink', 'gain life', 'whenever you gain']);
  ...
  },
  {
    ...
    cards: ['hexproof', 'aura'],
    score: 8,
    description: 'Hexproof creatures with auras are hard to remove and very powerful',
    example: 'Slippery Bogle + Rancor + Spider Umbra',
    minimumCards: 4,
    missingSuggestions: [
      {
        missing: 'Aura package',
        description: 'Your hexproof creatures need auras to maximize their potential',
        suggestion: ['Rancor', 'Spider Umbra', 'Ethereal Armor'],
        impact: 'high',
        condition: (deck) => {
          const hexproof = countKeyword(deck, 'hexproof');
          const auras = countType(deck, 'enchantment') + countPattern(deck, ['aura', 'enchant creature']);
  ...
  },
  {
    ...
    type: 'tribal',
    cards: ['Elf'],
    bonusCards: ['Heritage Druid', 'Wirewood', 'Ezuri', 'Craterhoof', 'Archdruid'],
    score: 9,
    ...
    missingSuggestions: [
      ...
      },
      {
        ...
        suggestion: ['Craterhoof Behemoth', 'Ezuri, Renegade Leader', 'Primeval Titan'],
        impact: 'high',
        condition: (deck) => {
          const elves = countPattern(deck, ['elf', 'elvish']);
    ...
    missingSuggestions: [
      {
        ...
        suggestion: ['Goblin Warchief', 'Goblin Chieftain', 'Muxus, Goblin Grandee'],
        impact: 'high',
        condition: (deck) => {
          const goblins = countPattern(deck, ['goblin']);
          const lords = countPattern(deck, ['warchief', 'chieftain', 'muxus', 'krenko', 'king']);
  ...
  // === MECHANIC SYNERGIES (4) ===
  {
    ...
    score: 7,
    description: 'Pump spells on evasive creatures connect more often for lethal damage',
    example: 'Mutagenic Growth on flying creature',
    minimumCards: 4,
    missingSuggestions: [
      {
        missing: 'Evasion sources',
        description: 'Your pump spells need evasive creatures to maximize damage',
        suggestion: ['Flying creatures', 'Unblockable creatures', 'Shadow creatures'],
        impact: 'medium',
        condition: (deck) => {
          const pump = countPattern(deck, ['pump', 'growth', 'might', 'strength', 'giant growth']);
          const evasion = countKeyword(deck, 'flying') + countKeyword(deck, 'unblockable');
    ...
    missingSuggestions: [
      {
        ...
        suggestion: ['Visera, Bloodchief', 'Zulaport Cutthroat', 'Phyrexian Altar'],
        impact: 'high',
        condition: (deck) => {
          const tokens = countPattern(deck, ['token', 'create creature', 'generate']);
          const outlet = countPattern(deck, ['sacrifice', 'offering', 'altar', 'visera']);
    ...
    missingSuggestions: [
      {
        ...
        impact: 'high',
        condition: (deck) => {
          const mill = countPattern(deck, ['mill', 'library', 'put into graveyard']);
          const wincon = countPattern(deck, ['jace', 'millstone', 'hedron crab', 'win']);
    ...
    missingSuggestions: [
      {
        ...
        suggestion: ['Blightsteel Colossus', 'Ulamog, the Ceaseless Hunger', 'Craterhoof Behemoth'],
        impact: 'high',
        condition: (deck) => {
          const ramp = countPattern(deck, ['ramp', 'mana', 'accelerate', 'cultivate', 'signet']);
    ...
    missingSuggestions: [
      {
        ...
        impact: 'medium',
        condition: (deck) => {
          const rocks = countPattern(deck, ['signet', 'stone', 'sol ring', 'mana rock']);
          const avgCmc = calculateAvgCmc(deck);
  ...
  },
  {
    ...
    missingSuggestions: [
      {
        missing: 'Fetch lands',
        description: 'Your landfall cards benefit from fetch lands for extra triggers',
        suggestion: ['Evolving Wilds', 'Terramorphic Expanse', 'Fetch lands'],
        impact: 'medium',
        condition: (deck) => {
          const landfall = countPattern(deck, ['landfall', 'whenever a land']);
          const fetch = countPattern(deck, ['fetch', 'evolving wilds', 'terramorphic']);
    ...
    missingSuggestions: [
      {
        ...
        condition: (deck) => {
          ...
          const ritual = countPattern(deck, ['ritual', 'dark ritual', 'cabal ritual', 'seething song']);
    ...
    missingSuggestions: [
      {
        ...
        suggestion: ['Basalt Monolith', 'Rings of Brighthearth', 'Power Artifact'],
        impact: 'high',
        condition: (deck) => {
    ...
    missingSuggestions: [
      {
        ...
        suggestion: ['Griselbrand', 'Archon of Cruelty', 'Atraxa, Grand Unifier'],
        impact: 'high',
        condition: (deck) => {
          const reanimate = countPattern(deck, ['reanimate', 'animate dead', 'necromancy', 'exhume']);
          const bigCreatures = deck.filter(c => (c.cmc || 0) >= 6 && c.type_line?.includes('Creature')).length;
          return reanimate >= 3 && bigCreatures < 4;
        },
      },
      {
        missing: 'Graveyard setup',
        description: 'Your reanimation needs ways to put creatures in graveyard',
        suggestion: ['Entomb', 'Buried Alive', 'Faithless Looting'],
        impact: 'high',
        condition: (deck) => {
          const reanimate = countPattern(deck, ['reanimate', 'animate dead', 'necromancy']);
          const setup = countPattern(deck, ['entomb', 'buried', 'mill', 'loot', 'discard']);
    ...
    missingSuggestions: [
      {
        ...
        suggestion: ['Thassa, Deep-Dwelling', 'Oracle of Mul Daya', 'Village Rites'],
        impact: 'high',
        condition: (deck) => {
          const hulk = countPattern(deck, ['protean hulk']);
          const targets = countPattern(deck, ['thassa', 'oracle', 'village rites', 'dies']);
    ...
    missingSuggestions: [
      {
        ...
        suggestion: ['Kiki-Jiki, Mirror Breaker', 'Pestermite', 'Deceiver Exarch'],
        impact: 'high',
        condition: (deck) => {
          const kiki = countPattern(deck, ['kiki-jiki', 'kiki']);
          const exarch = countPattern(deck, ['pestermite', 'deceiver exarch', 'exarch']);
    ...
    missingSuggestions: [
      {
        ...
        suggestion: ['Intangible Virtue', 'Anointed Procession', 'Parallel Lives'],
        impact: 'medium',
        condition: (deck) => {
          const tokens = countPattern(deck, ['token', 'create creature']);
          const anthem = countPattern(deck, ['anthem', 'virtue', 'procession', 'lives', '+1/+1']);
    ...
    missingSuggestions: [
      {
        ...
        suggestion: ['Master of Etherium', 'Thopter Foundry', 'Arcbound Ravager'],
        impact: 'high',
        condition: (deck) => {
          const artifacts = countType(deck, 'artifact');
          const lords = countPattern(deck, ['master of etherium', 'thopter', 'arcbound']);
    ...
    missingSuggestions: [
      {
        ...
        suggestion: ['Eidolon of Blossoms', 'Sigil of the Empty Throne', 'Enchantress\'s Presence'],
        impact: 'high',
        condition: (deck) => {
          const enchantments = countType(deck, 'enchantment');
          const draw = countPattern(deck, ['enchantress', 'eidolon', 'sigil', 'draw']);
    ...
    missingSuggestions: [
      {
        ...
        suggestion: ['Teferi\'s Protection', 'Heroic Intervention', 'Counterspells'],
        impact: 'high',
        condition: (deck) => {
          const walkers = countType(deck, 'planeswalker');
          const protection = countPattern(deck, ['protection', 'heroic intervention', 'counterspell']);
    ...
    missingSuggestions: [
      {
        ...
        description: 'Your planeswalker deck needs The Chain Veil for extra activations',
