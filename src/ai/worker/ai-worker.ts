...
const aiWorker: AIWorkerAPI = {
  ...
  async prepareCoachContext(payload: CoachContextPayload): Promise<DigestedCoachContext> {
    ...
    const deckSummary = cards.length > 0 ? {
      ...
      typeCounts: cards.reduce((acc, card) => {
        const type = card.type_line.split('—')[0].trim().split(' ')[0];
        acc[type] = (acc[type] || 0) + (card.count || 1);
        return acc;
      }, {} as Record<string, number>),
      averageCmc: cards.length > 0 ? cards.reduce((sum, c) => sum + (c.cmc || 0) * (c.count || 1), 0) / cards.reduce((sum, c) => sum + (c.count || 1), 0) : 0,
      keyCards: cards
        .filter(c => (c.cmc || 0) >= 5 || (c.type_line.includes('Planeswalker')))
        .sort((a, b) => (b.cmc || 0) - (a.cmc || 0))
        .slice(0, 5)
        .map(c => c.name),
      manaCurve: Array.from({ length: 8 }, (_, i) => 
        cards.filter(c => (c.cmc === i || (i === 7 && (c.cmc || 0) >= 7)) && !c.type_line.includes('Land'))
    ...
    const gameSummary = gameState ? {
      ...
      players: Object.entries(gameState.players).map(([id, player]) => ({
        ...
        manaAvailable: player.manaPool ? Object.values(player.manaPool).reduce((a, b) => a + b, 0) : 0,
        keyPermanents: player.battlefield
          .filter(c => (c.manaValue || 0) >= 4 || c.type === 'planeswalker' || (c.keywords && c.keywords.includes('Legendary')))
          .map(c => c.name)
          .slice(0, 3)
      }))
    } : undefined;
