...
describe("getAIModel", () => {
  ...
  beforeEach(() => {
    ...
    (createOpenAI as jest.Mock).mockReturnValue(
      jest.fn().mockReturnValue(mockModel),
    );
    (createAnthropic as jest.Mock).mockReturnValue(
      jest.fn().mockReturnValue(mockModel),
    );
    (createGoogleGenerativeAI as jest.Mock).mockReturnValue(
      jest.fn().mockReturnValue(mockModel),
    );
  ...
  it("should route legacy zaic through the OpenAI SDK with ZAI credentials", async () => {
    process.env.ZAI_API_KEY = "test-zai-key";
    const mockZaic = jest.fn().mockReturnValue(mockModel);
    (createOpenAI as jest.Mock).mockReturnValue(mockZaic);

    const result = await getAIModel("zaic");
    expect(createOpenAI).toHaveBeenCalledWith({
      apiKey: "test-zai-key",
      baseURL: "https://api.z-ai.com/v1",
    });
    expect(mockZaic).toHaveBeenCalledWith(PROVIDER_DEFAULT_MODELS.zaic);
    expect(result).toBe(mockModel);
  });

  it("should treat 'default' as the zaic default model id", async () => {
    const mockZaic = jest.fn().mockReturnValue(mockModel);
    (createOpenAI as jest.Mock).mockReturnValue(mockZaic);

    await getAIModel("zaic", "default");
    expect(mockZaic).toHaveBeenCalledWith(PROVIDER_DEFAULT_MODELS.zaic);
  });
