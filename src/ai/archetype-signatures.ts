    ...
    keywords.forEach((keyword) => {
      ...
      if (matches) {
        keywordCounts[keyword] =
          (keywordCounts[keyword] || 0) + matches.length * count;
      }
    });
  }
...
export const ARCHETYPE_SIGNATURES: ArchetypeSignature[] = [
  // === AGGRO ARCHETYPES (3) ===
  {
    ...
    scoreFunction: (deck, stats) => {
      let score = 0;
      const burnSpells = countPatternMatches(deck, [
        "lightning bolt",
        "lava spike",
        "skewer",
        "burst lightning",
  ...
  },
  {
    ...
    category: "aggro",
    description:
      "Efficient creatures with pump spells for fast aggressive wins",
    ...
    scoreFunction: (deck, stats) => {
      let score = 0;
      const pumpSpells = countPatternMatches(deck, [
        "pump",
        "giant growth",
        "mutagenic growth",
        "berserk",
      ...
      if (stats.creatureRatio >= 0.5 && stats.creatureRatio <= 0.75)
        score += 35;
  ...
  // === CONTROL ARCHETYPES (3) ===
  {
    ...
    scoreFunction: (deck, stats) => {
      let score = 0;
      const counterspells = countPatternMatches(deck, [
        "counterspell",
        "force of will",
        "mana drain",
        "mystic force",
      ...
      const cardDraw = countPatternMatches(deck, [
        "draw",
        "concentrate",
        "ancestral",
        "brainstorm",
      ...
      const instantRatio =
        (stats.cardTypes.instant || 0) / Math.max(stats.totalCards, 1);
  ...
  },
  {
    ...
    scoreFunction: (deck, stats) => {
      ...
      const lockPieces = countPatternMatches(deck, [
        "static orb",
        "stasis",
        "winter orb",
        "smokestack",
  ...
  },
  {
    ...
    category: "control",
    description:
      "Lock opponents out of the game with permanent-based restrictions",
    minCreatureRatio: 0.1,
    ...
    scoreFunction: (deck, stats) => {
      ...
      const removal = countPatternMatches(deck, [
        "destroy",
        "exile",
        "banish",
        "oblivion",
  ...
  // === MIDRANGE ARCHETYPES (3) ===
  {
    ...
    category: "midrange",
    description:
      "Collection of individually powerful cards with balanced curve",
    ...
    scoreFunction: (deck, stats) => {
      ...
      if (stats.creatureRatio >= 0.35 && stats.creatureRatio <= 0.55)
        score += 30;
      ...
      // Balanced mana curve
      const curveBalance =
        Math.abs(stats.manaCurve[2] - stats.manaCurve[3]) < 5 &&
  ...
  },
  {
    ...
    scoreFunction: (deck, stats) => {
      ...
      const ramp = countPatternMatches(deck, [
        "ramp",
        "mana",
        "accelerate",
        "cultivate",
  ...
  },
  {
    ...
    scoreFunction: (deck, stats) => {
      let score = 0;
      const cardAdvantage = countPatternMatches(deck, [
        "draw",
        "advantage",
        "value",
        "engine",
  ...
  // === COMBO ARCHETYPES (3) ===
  {
    ...
    category: "combo",
    description:
      "Cast many spells in one turn to build storm count for game-winning finish",
    minCreatureRatio: 0,
    maxCreatureRatio: 0.2,
    avgCmcRange: [1.5, 3.0],
    cardPatterns: [
    ...
    scoreFunction: (deck, stats) => {
      let score = 0;
      const cantrips = countPatternMatches(deck, [
        "draw",
        "cantrip",
        "opt",
        "peek",
      ...
      const rituals = countPatternMatches(deck, [
        "ritual",
        "dark ritual",
        "cabal ritual",
        "seething song",
  ...
  },
  {
    ...
    category: "combo",
    description:
      "Put big creatures into graveyard then return them to battlefield",
    ...
    scoreFunction: (deck, stats) => {
      let score = 0;
      const reanimation = countPatternMatches(deck, [
        "reanimate",
        "animate dead",
        "necromancy",
        "exhume",
      ...
      const graveyardSetup = countPatternMatches(deck, [
        "entomb",
        "buried",
        "mill",
        "discard",
      ...
      if (stats.creatureRatio >= 0.25 && stats.creatureRatio <= 0.6)
        score += 15;
  ...
  },
  {
    ...
    scoreFunction: (deck, stats) => {
      let score = 0;
      const comboPieces = countPatternMatches(deck, [
        "infinite",
        "combo",
        "loop",
        "thassa",
      ...
      const tutors = countPatternMatches(deck, [
        "tutor",
        "search",
        "fetch",
        "find",
      ...
      const protection = countPatternMatches(deck, [
        "protection",
        "counterspell",
        "deflect",
        "foil",
  ...
  // === TRIBAL ARCHETYPES (4) ===
  {
    ...
    scoreFunction: (deck, stats) => {
      ...
      const lords = countPatternMatches(deck, [
        "lord",
        "archdruid",
        "imperator",
        "ezuri",
  ...
  },
  {
    ...
    scoreFunction: (deck, stats) => {
      ...
      const sacrifice = countPatternMatches(deck, [
        "sacrifice",
        "offering",
        "munitions",
        "explosion",
  ...
  },
  {
    ...
    scoreFunction: (deck, stats) => {
      let score = 0;
      const zombies = countPatternMatches(deck, [
        "zombie",
        "lich",
        "skeleton",
        "undead",
      ...
      const graveyard = countPatternMatches(deck, [
        "graveyard",
        "recur",
        "return",
        "rise",
  ...
  // === SPECIAL ARCHETYPES (2) ===
  {
    ...
    scoreFunction: (deck, stats) => {
      ...
      const landRecursion = countPatternMatches(deck, [
        "crucible",
        "ramunap",
        "witness",
        "regrowth",
      ...
      const landUtility = countPatternMatches(deck, [
        "manland",
        "utility",
        "bojuka",
        "castle",
  ...
  // Score thresholds tuned lower to catch hybrid decks
  {
    ...
    scoreFunction: (deck, stats) => {
      let score = 0;
      const burn = countPatternMatches(deck, [
        "lightning bolt",
        "lava spike",
        "skewer",
        "burst",
      ...
      if (stats.creatureRatio >= 0.3 && stats.creatureRatio <= 0.55)
        score += 30;
  ...
  },
  {
    ...
    category: "control",
    description:
      "Control shell with tempo elements and cheap interactive spells",
    ...
    scoreFunction: (deck, stats) => {
      let score = 0;
      const counters = countPatternMatches(deck, [
        "counterspell",
        "force of will",
        "mana drain",
        "negate",
      ...
      const tempo = countPatternMatches(deck, [
        "snapcaster",
        "spell queller",
        "brazen",
        "borrower",
      ...
      if (stats.creatureRatio >= 0.15 && stats.creatureRatio <= 0.35)
        score += 20;
  ...
  },
  {
    ...
    scoreFunction: (deck, stats) => {
      let score = 0;
      const pumpBurn = countPatternMatches(deck, [
        "pump",
        "giant growth",
        "mutagenic",
        "berserk",
      ...
      const threats = countPatternMatches(deck, [
        "threat",
        "finisher",
        "titan",
        "goyf",
  ...
  },
  {
    ...
    scoreFunction: (deck, stats) => {
      ...
      const sweeper = countPatternMatches(deck, [
        "wrath",
        "damnation",
        "toxic deluge",
        "engineered",
  ...
  },
  {
    ...
    scoreFunction: (deck, stats) => {
      let score = 0;
      const discard = countPatternMatches(deck, [
        "inquisition",
        "thoughtseize",
        " Collective",
        "blightning",
      ...
      const bigThreats = countPatternMatches(deck, [
        "tarmogoyf",
        "goyf",
        "troll",
        "scavenging",
