...
describe("Game Code Generation", () => {
  ...
  // Issue #1906 — mirror the canonical CSPRNG contract from
  // src/app/api/signaling/route.ts:80. The static generateGameCode()
  // must HARD-FAIL when crypto.getRandomValues is unavailable; a silent
  // Math.random() fallback is non-uniform and predictable and would
  // let a peer enumerate candidate lobby codes.
  describe("CSPRNG contract (issue #1906)", () => {
    ...
    it("THROWS the typed error when globalThis.crypto is undefined", () => {
      Object.defineProperty(globalThis, "crypto", {
        value: undefined,
        configurable: true,
        writable: true,
      });
      expect(() => LocalSignalingClient.generateGameCode()).toThrow(
    ...
    it("THROWS when crypto exists but lacks getRandomValues", () => {
      ...
      try {
        expect(() => LocalSignalingClient.generateGameCode(8)).toThrow(
          /crypto\.getRandomValues/,
        );
      } finally {
        Object.defineProperty(globalThis.crypto, "getRandomValues", {
...
describe("Connection String Parsing", () => {
  ...
  it("should return null for malformed JSON", () => {
    const malformed =
      LocalSignalingClient.parseConnectionString("{ invalid json }");
