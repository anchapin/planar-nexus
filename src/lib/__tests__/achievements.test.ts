...
const mockGameState = {
  players: new Map([
    ['player-1', { id: 'player-1', name: 'Test Player', life: 20, hand: [], battlefield: [], library: [], graveyard: [] }],
  ]),
  format: 'standard',
...
describe('Achievements Module', () => {
  describe('ACHIEVEMENTS', () => {
    ...
    it('should have valid rarity for all achievements', () => {
      const validRarities: AchievementRarity[] = ['common', 'uncommon', 'rare', 'epic', 'legendary'];
    ...
    it('should have valid category for all achievements', () => {
      const validCategories: AchievementCategory[] = ['games', 'wins', 'collection', 'social', 'special'];
      ACHIEVEMENTS.forEach(achievement => {
    ...
    it('should have collection achievements', () => {
      const collectionAchievements = ACHIEVEMENTS.filter(a => a.requirement.type === 'cards_collected');
      expect(collectionAchievements.length).toBeGreaterThan(0);
    });

    it('should have format-specific achievements', () => {
      const formatAchievements = ACHIEVEMENTS.filter(a => a.requirement.type === 'games_with_format');
...
describe('AchievementManager', () => {
  ...
  describe('getPlayerAchievements', () => {
    it('should return player achievements structure', async () => {
      const playerData = await (achievementManager as any).getPlayerAchievements(testPlayerId);
  ...
  describe('checkGameAchievements', () => {
    ...
    it('should check first_game achievement after playing a game', async () => {
      ...
      const gameState = {
        players: new Map([
          [uniquePlayerId, { id: uniquePlayerId, name: 'Test Player', life: 20, hand: [], battlefield: [], library: [], graveyard: [] }],
        ]),
        format: 'standard',
    ...
    it('should check first_win achievement after winning a game', async () => {
      ...
      const gameState = {
        players: new Map([
          [testPlayerId, { id: testPlayerId, name: 'Test Player', life: 20, hand: [], battlefield: [], library: [], graveyard: [] }],
        ]),
        format: 'standard',
    ...
    it('should check comeback_win achievement when winning with low life', async () => {
      ...
      const gameState = {
        players: new Map([
          [testPlayerId, { id: testPlayerId, name: 'Test Player', life: 3, hand: [], battlefield: [], library: [], graveyard: [] }],
        ]),
        format: 'standard',
  ...
  describe('getUnlockedAchievements', () => {
    it('should return empty array when no achievements unlocked', async () => {
      const unlocked = await achievementManager.getUnlockedAchievements(testPlayerId);
    ...
    it('should return unlocked achievements', async () => {
      ...
      await achievementManager.checkGameAchievements(testPlayerId, gameState as any, true);
  ...
  describe('getAchievementDisplayProgress', () => {
    it('should return progress for all achievements', async () => {
      const displayProgress = await achievementManager.getAchievementDisplayProgress(testPlayerId);
  ...
  describe('subscribe', () => {
    ...
    it('should call subscriber when achievement is unlocked', async () => {
      ...
      await achievementManager.checkGameAchievements(testPlayerId, gameState as any, false);
  ...
  describe('resetAchievements', () => {
    it('should reset achievements for a player', async () => {
      ...
      let unlocked = await achievementManager.getUnlockedAchievements(testPlayerId);
      expect(unlocked.length).toBeGreaterThan(0);
...
describe('Achievement Categories', () => {
  it('should have games category achievements', () => {
    const gamesAchievements = ACHIEVEMENTS.filter(a => a.category === 'games');
...
describe('Achievement Progression', () => {
  it('should have progressive milestones for games played', () => {
    const gamesAchievements = ACHIEVEMENTS
      .filter(a => a.requirement.type === 'games_played')
  ...
  it('should have progressive milestones for wins', () => {
    const winsAchievements = ACHIEVEMENTS
      .filter(a => a.requirement.type === 'wins')
      .sort((a, b) => a.requirement.count - b.requirement.count);
  ...
  it('should have progressive milestones for collection', () => {
    const collectionAchievements = ACHIEVEMENTS
      .filter(a => a.requirement.type === 'cards_collected')
      .sort((a, b) => a.requirement.count - b.requirement.count);
...
describe('Achievement Points Distribution', () => {
  ...
  it('should award more points for higher rarity', () => {
    ...
    const commonAvgPoints = commonAchievements.reduce((sum, a) => sum + a.points, 0) / commonAchievements.length;
    const legendaryAvgPoints = legendaryAchievements.reduce((sum, a) => sum + a.points, 0) / legendaryAchievements.length;
