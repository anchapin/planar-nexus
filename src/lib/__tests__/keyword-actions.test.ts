...
import type { GameState, CardInstance, CardInstanceId, PlayerId, ScryfallCard } from '@/lib/game-state/types';
import { createToken } from '../game-state/card-instance';
import { getHandFilterForCard, hasIndestructible, canBeRegenerated } from '../game-state/keyword-actions';
...
function createMockGameState(overrides: Partial<GameState> = {}): GameState {
  ...
  return {
    ...
    status: 'pending',
    players: new Map([
      [player1Id, { id: player1Id, name: 'Player 1', life: 20, hand: [], battlefield: [], library: [], graveyard: [], exile: [], sideboard: [] }],
      [player2Id, { id: player2Id, name: 'Player 2', life: 20, hand: [], battlefield: [], library: [], graveyard: [], exile: [], sideboard: [] }],
    ]),
    zones: new Map([
      ['player1-hand', []],
      ['player1-battlefield', []],
    ...
    cards: new Map(),
    turn: { turnNumber: 1, currentPlayer: player1Id, phase: 'untap', step: 'upkeep' },
    priority: player1Id,
    stack: [],
    effectStack: [],
    replacementEffects: [],
    appliedReplacementEffects: new Map(),
    mana: new Map([[player1Id, new Map()], [player2Id, new Map()]]),
...
describe('canBeRegenerated', () => {
  it('should return true for regenerate mechanic', () => {
    const card = createMockCardInstance({ oracle_text: '{T}: Regenerate this creature.' });
...
describe('createToken', () => {
  it('should create a token card', () => {
    const token = createToken(
      { name: '1/1 white Soldier creature token', type_line: 'Creature — Soldier', colors: ['white'] } as ScryfallCard,
      'player1',
      'player1',
    );
  ...
  it('should handle different token types', () => {
    const zombieToken = createToken(
      { name: '2/2 black Zombie creature token', type_line: 'Creature — Zombie', colors: ['black'] } as ScryfallCard,
      'player1',
      'player1',
    );
    const treasureToken = createToken(
      { name: 'Treasure', type_line: 'Artifact — Treasure', colors: [] } as ScryfallCard,
      'player1',
      'player1',
    );
    const clueToken = createToken(
      { name: 'Clue', type_line: 'Artifact — Clue', colors: [] } as ScryfallCard,
      'player1',
      'player1',
    );
