...
describe("IndexedDB Storage", () => {
  ...
  describe("Error Handling", () => {
    ...
    it("should handle delete operation for non-existent key", async () => {
      ...
      await expect(
        storage.delete("test-decks", "non-existent"),
      ...
      const largeDeck = {
        ...
        cards: Array.from({ length: 100 }, (_, i) => ({
          card: {
            ...
            name: `Card ${i}`,
            cmc: i % 7,
            colors: ["white", "blue", "black", "red", "green"].slice(
              0,
              (i % 5) + 1,
            ),
            color_identity: [],
  ...
  describe("Database Upgrade/Migration", () => {
    ...
    it("should create indexes on upgrade", async () => {
      ...
      const results = await indexedStorage.queryByIndex<any>(
        "decks",
        "format",
        "standard",
      );
  ...
  describe("GetStorage", () => {
    it("should return default storage instance", async () => {
      const storage = await import("../indexeddb-storage").then((m) =>
        m.getStorage(),
      );
      expect(storage).toBeDefined();
    });
...
describe("BackupManifest tracking (Phase 34 Plan 01)", () => {
  let storage: IndexedDBStorage;

  beforeEach(async () => {
    storage = new IndexedDBStorage({
      dbName: "TestManifestStorage",
      version: 3,
      stores: [
        "decks",
        "saved-games",
  ...
  it("exportBackup() updates lastFullBackupAt in manifest", async () => {
    await storage.exportBackup();
    const manifest = await storage.getBackupManifest();
    expect(manifest).not.toBeNull();
    expect(manifest!.lastFullBackupAt).toBeTruthy();
  });

  it("importBackup() still works correctly (no regression)", async () => {
    const backup = await storage.exportBackup();
    await expect(storage.importBackup(backup)).resolves.not.toThrow();
  });
});
