...
import { getEffectiveToughness } from "./layer-system";
  ...
  try {
    ...
    for (const [playerId, player] of updatedState.players) {
      ...
      if (player.poisonCounters >= 10) {
        ...
        descriptions.push(
          `${player.name} loses the game (10+ poison counters)`,
        );
        actionsPerformed = true;
      }
    ...
    for (const card of cardsToCheck) {
      ...
      if (isOnBf) {
        ...
        if (isCreature(card)) {
          ...
          // This ensures creatures whose toughness is set to 0 by layer effects
          // (color changes, type changes, P/T setting effects) are properly
          // destroyed by this SBA.
          const effectiveToughness = getEffectiveToughness(
            card,
            updatedState.layerSystem,
          );
    ...
    for (const pw of planeswalkers) {
      ...
      const pwType = typeLine
        .replace(/Legendary Planeswalker [-—] /i, "")
        .trim();
    ...
    return {
      ...
      state: originalState,
      descriptions: [
        `SBA check failed: ${err instanceof Error ? err.message : String(err)}`,
      ],
    };
  }
}
