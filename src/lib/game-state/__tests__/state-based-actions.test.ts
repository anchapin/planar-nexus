...
describe("State-Based Actions", () => {
  describe("Player Life Total (SBA 704.5a)", () => {
    it("should make a player lose when life reaches 0", () => {
      ...
      const alice = result.state.players.get(
        Array.from(state.players.keys())[0],
      );
  ...
  describe("Poison Counters (SBA 704.5b)", () => {
    it("should make a player lose with 10 or more poison counters", () => {
      ...
      expect(updatedAlice?.lossReason).toBe(
        "Accumulated 10 or more poison counters",
      );
    });
  ...
  describe("+1/+1 and -1/-1 Counter Cancellation (SBA 704.5q)", () => {
    it("should remove equal number of +1/+1 and -1/-1 counters", () => {
      ...
      const plusOneCounters = updatedCreature.counters.find(
        (c) => c.type === "+1/+1",
      );
      const minusOneCounters = updatedCreature.counters.find(
        (c) => c.type === "-1/-1",
      );
    ...
    it("should handle multiple counter types correctly", () => {
      ...
      const chargeCounters = updatedCreature.counters.find(
        (c) => c.type === "charge",
      );
  ...
  describe("Planeswalker Uniqueness Rule", () => {
    it("should destroy duplicate planeswalkers of the same type", () => {
      ...
      const jaceOnBattlefield = battlefieldAfter.cardIds.filter(
        (id) => id === jace1.id || id === jace2.id,
      );
      const jaceInGraveyard = graveyard.cardIds.filter(
        (id) => id === jace1.id || id === jace2.id,
      );
  ...
  describe("Win Condition Checking", () => {
    ...
    it("should end the game as a draw when all players lose", () => {
      ...
      expect(result.state.endReason).toBe(
        "All players lost the game simultaneously",
      );
    });
  });
  ...
  describe("Combat Damage Integration", () => {
    it("should destroy creatures with lethal damage after combat", () => {
      ...
      const aliceCreature = createCardInstance(
        aliceCreatureData,
        aliceId,
        aliceId,
      );
    ...
    it("should handle deathtouch lethal damage", () => {
      ...
      const deathtouchCreatureData = createMockCreature(
        "Deathtouch Creature",
        1,
        1,
        ["Deathtouch"],
      );
      const deathtouchCreature = createCardInstance(
        deathtouchCreatureData,
        aliceId,
        aliceId,
      );
      ...
      state = dealDamageToCard(
        state,
        bigCreature.id,
        1,
        true,
        deathtouchCreature.id,
      ).state;
  ...
  describe("Multiple SBA Simultaneous Resolution", () => {
    it("should handle multiple creatures dying simultaneously", () => {
      ...
      const creature3 = createCardInstance(
        creature3Data,
        playerIds[0],
        playerIds[0],
      );
  ...
  describe("SBA Integration with Priority Loop (CR 704.5j)", () => {
    ...
    it("should trigger SBA when player life reaches 0 after both players pass", () => {
      ...
      // First pass from active player (Alice) who has priority initially
      state = passPriority(state, aliceId);
      // Second pass from non-active player (Bob)
    ...
    it("should trigger multiple SBAs in correct order after priority pass", () => {
      ...
      const creatureData1 = createMockCreature(
        "Duplicated Legend",
        2,
        2,
        [],
      ...
      const creatureData2 = createMockCreature(
        "Duplicated Legend",
        2,
        2,
        [],
