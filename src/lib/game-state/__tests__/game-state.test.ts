...
describe('Game State Management', () => {
  ...
  describe('loadDeckForPlayer', () => {
    ...
    it('should shuffle the library', () => {
      ...
      const deckCards = Array(60).fill(null).map((_, i) => 
        createMockCard(`Card${i}`, 'Instant', i % 5)
      );
  ...
  describe('concede', () => {
    ...
    it('should end game when only one player remains', () => {
      ...
      expect(gameState.winners).toEqual([Array.from(gameState.players.keys())[1]]);
    });
  ...
  describe('Draw offers', () => {
    ...
    describe('offerDraw', () => {
      ...
      it('should throw error for game not in progress', () => {
        const notStartedState = createInitialGameState(['Player 1', 'Player 2']);
  ...
  describe('Edge cases', () => {
    it('should handle very large life totals', () => {
      ...
      const player = gameState.players.get(Array.from(gameState.players.keys())[0]);
