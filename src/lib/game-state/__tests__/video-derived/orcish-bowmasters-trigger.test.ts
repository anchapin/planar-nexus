...
describe('Video-Derived Fixture: orcish-bowmasters-trigger', () => {
  ...
  it('has valid player data', () => {
    ...
    expect(orcish_bowmasters_trigger.gameState.opponent_life).toBeGreaterThan(0);
    expect(Array.isArray(orcish_bowmasters_trigger.gameState.battlefield_player)).toBe(true);
    expect(Array.isArray(orcish_bowmasters_trigger.gameState.battlefield_opponent)).toBe(true);
  });

  it('has valid turn structure', () => {
    expect(typeof orcish_bowmasters_trigger.gameState.turn_number).toBe('number');
