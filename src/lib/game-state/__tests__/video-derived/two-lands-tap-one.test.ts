/**
 * Video-Derived Test Fixture: two-lands-tap-one
...
describe('Video-Derived Fixture: two-lands-tap-one', () => {
  it('loads game state successfully', () => {
    expect(two_lands_tap_one.gameState).toBeDefined();
    expect(two_lands_tap_one.gameState).toBeInstanceOf(Object);
  });

  it('has valid player data', () => {
    expect(two_lands_tap_one.gameState.player_life).toBeGreaterThan(0);
    expect(two_lands_tap_one.gameState.opponent_life).toBeGreaterThan(0);
    expect(Array.isArray(two_lands_tap_one.gameState.battlefield_player)).toBe(true);
    expect(Array.isArray(two_lands_tap_one.gameState.battlefield_opponent)).toBe(true);
  });
