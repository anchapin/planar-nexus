...
describe("Spell Casting - Timing Restrictions", () => {
  describe("canCastSpell", () => {
    it("should allow casting instant at any time with priority", () => {
      const { state, aliceId, cardId } = setupGameWithCardInHand(
        createMockInstant("Counterspell"),
      );
    ...
    it("should allow casting sorcery during main phase with empty stack on own turn", () => {
      const { state, aliceId, cardId } = setupGameWithCardInHand(
        createMockSorcery("Lightning Bolt"),
      );
    ...
    it("should not allow casting sorcery on opponent's turn", () => {
      const { state, aliceId, bobId, cardId } = setupGameWithCardInHand(
        createMockSorcery("Lightning Bolt"),
      );
    ...
    it("should allow casting creature with flash at instant speed", () => {
      const { state, aliceId, cardId } = setupGameWithCardInHand(
        createMockFlashCreature("Flash Creature"),
      );
...
describe("Spell Casting - Casting Process", () => {
  describe("castSpell", () => {
    ...
    it("should spend mana when casting", () => {
      const { state, aliceId, cardId } = setupGameWithCardInHand(
        createMockInstant("Counterspell", "{1}{U}"),
      );
      ...
      const manaBefore =
        playerBefore.manaPool.blue + playerBefore.manaPool.generic;
      ...
      const manaAfter =
        playerAfter.manaPool.blue + playerAfter.manaPool.generic;
...
describe("Spell Casting - Stack Resolution", () => {
  describe("resolveTopOfStack", () => {
    ...
    it("should move creature to battlefield after resolution", () => {
      const { state, aliceId, cardId } = setupGameWithCardInHand(
        createMockCreature("Grizzly Bears", 2, 2),
      );
...
describe("Spell Casting - Edge Cases", () => {
  ...
  it("should handle flash creatures correctly during combat", () => {
    const { state, aliceId, cardId } = setupGameWithCardInHand(
      createMockFlashCreature("Ambush Viper"),
    );
...
describe("Spell Casting - Modal Spell Choice (choose_mode)", () => {
  describe("resolveWaitingChoice with choose_mode", () => {
    it("should resolve mode choice and update chosenModes on stack object", () => {
      let state = createInitialGameState(["Alice", "Bob"], 20, false);
      state = startGame(state);
      ...
      // Create a modal spell on the stack
      const stackObj: StackObject = {
        ...
        text: "Choose one - Deal 3 damage; or Draw a card; or Create a 1/1 white Soldier creature token.",
        manaCost: "{2}{W}",
        targets: [],
        chosenModes: [],
        variableValues: new Map(),
      ...
      // Set up waiting choice for mode selection
      state.waitingChoice = {
        type: "choose_mode",
        playerId: aliceId,
        stackObjectId: "modal-spell-1",
        prompt: "Choose a mode for Valiant Veteran:",
        choices: [
          { label: "Deal 3 damage", value: "Deal 3 damage", isValid: true },
          { label: "Draw a card", value: "Draw a card", isValid: true },
          {
            label: "Create a 1/1 white Soldier creature token",
            value: "Create a 1/1 white Soldier creature token",
            isValid: true,
          },
        ],
        minChoices: 1,
      ...
      // Resolve with first mode
      const result = resolveWaitingChoice(state, aliceId, "Deal 3 damage");

      expect(result.success).toBe(true);
      expect(result.state.waitingChoice).toBeNull();

      // Check that chosenModes was updated
      const updatedStackObj = result.state.stack.find(
        (s) => s.id === "modal-spell-1",
      );
      expect(updatedStackObj?.chosenModes).toContain("Deal 3 damage");
    });

    it("should reject resolveWaitingChoice when no waiting choice exists", () => {
      let state = createInitialGameState(["Alice"], 20, false);
      state = startGame(state);
      ...
      const result = resolveWaitingChoice(state, aliceId, "some mode");

      expect(result.success).toBe(false);
      expect(result.error).toBe("No waiting choice to resolve");
    });

    it("should reject resolveWaitingChoice when wrong player tries to choose", () => {
      let state = createInitialGameState(["Alice", "Bob"], 20, false);
      state = startGame(state);
      ...
      state.waitingChoice = {
        ...
        choices: [{ label: "Mode 1", value: "mode1", isValid: true }],
        minChoices: 1,
        maxChoices: 1,
        presentedAt: Date.now(),
      };

      // Bob tries to resolve when Alice should be choosing
      const result = resolveWaitingChoice(state, bobId, "mode1");

      expect(result.success).toBe(false);
      expect(result.error).toContain("Not this player's turn");
    });

    it("should handle mode choice with numeric values", () => {
      let state = createInitialGameState(["Alice"], 20, false);
      state = startGame(state);
      ...
      state.waitingChoice = {
        type: "choose_mode",
        playerId: aliceId,
        stackObjectId: "modal-spell-numeric",
        prompt: "Choose a mode:",
        choices: [
          { label: "Mode 1", value: 0, isValid: true },
          { label: "Mode 2", value: 1, isValid: true },
        ],
        minChoices: 1,
        maxChoices: 1,
        presentedAt: Date.now(),
      ...
      const result = resolveWaitingChoice(state, aliceId, 1);

      expect(result.success).toBe(true);
      const updatedStackObj = result.state.stack.find(
        (s) => s.id === "modal-spell-numeric",
      );
      expect(updatedStackObj?.chosenModes).toContain("1");
    });
  });
});
