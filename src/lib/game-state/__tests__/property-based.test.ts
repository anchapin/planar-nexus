...
describe("Property-based Tests: Game Invariants", () => {
  ...
  function buildStateWithStack(stack: StackObject[]): GameState {
    ...
    const normalizedStack = stack.map((obj) => ({
      ...obj,
      controllerId: firstPlayer,
    }));
  ...
  describe("Property-based Tests: Stack Resolution Determinism", () => {
        ...
        fc.property(
          ...
          (specs) => {
            ...
            for (let r = 0; r < REPETITIONS; r++) {
              hashes.push(
                computeStateHash(resolveEntireStack(cloneGameState(base))),
              );
            }
            ...
            expect(resolveEntireStack(cloneGameState(base)).stack.length).toBe(
              0,
            );
          },
        ),
    ...
    it("spell resolution order is deterministic and order-dependent (fc.tuple)", () => {
      fc.assert(
        fc.property(fc.tuple(spellSpecArb, spellSpecArb), ([specP, specQ]) => {
          ...
          ).toBe(computeStateHash(resolveEntireStack(cloneGameState(forward))));
          ...
          expect(forwardAfterOne.stack[0].id).not.toBe(
            reverseAfterOne.stack[0].id,
          );

          // Despite differing removal order, fully resolving either yields an
          // empty stack.
          expect(resolveEntireStack(cloneGameState(forward)).stack.length).toBe(
            0,
          );
          expect(resolveEntireStack(cloneGameState(reverse)).stack.length).toBe(
            0,
          );
        }),
        { numRuns: 100 },
      ...
      fc.assert(
        fc.property(
          ...
          (entries) => {
            ...
            // Correctness (CR 701.5, issue #1578): each countered object is
            // removed from the stack immediately; non-countered objects remain.
            for (let i = 0; i < entries.length; i++) {
              ...
              if (entries[i].counter) {
                ...
                expect(obj!.isCountered).toBe(false);
              }
            }

            // Determinism: two peers reach the identical resolved state.
            const hashA = computeStateHash(
              resolveEntireStack(cloneGameState(state)),
            );
            const hashB = computeStateHash(
              resolveEntireStack(cloneGameState(state)),
            );
            expect(hashA).toBe(hashB);

            // Full resolution always empties the stack, countered or not.
            expect(resolveEntireStack(cloneGameState(state)).stack.length).toBe(
              0,
            );
          },
        ),
