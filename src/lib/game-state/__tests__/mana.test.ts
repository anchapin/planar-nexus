...
describe("Mana Cost Parsing", () => {
  ...
  describe("parseManaAbility", () => {
    it("should parse basic land mana ability", () => {
      const options = parseManaAbility("{T}: Add {W}.");
      expect(options).toHaveLength(1);
      expect(options[0].mana).toEqual({ white: 1 });
    });

    it("should parse dual-color land (Hallowed Fountain)", () => {
      const options = parseManaAbility("{T}: Add {W} or {U}.");
      expect(options).toHaveLength(2);
      expect(options[0].mana).toEqual({ white: 1 });
      expect(options[1].mana).toEqual({ blue: 1 });
    });

    it("should parse five-color land (any color)", () => {
      const options = parseManaAbility("{T}: Add one mana of any color.");
    ...
    it("should parse colorless mana", () => {
      const options = parseManaAbility("{T}: Add {C}.");
      expect(options).toHaveLength(1);
      expect(options[0].mana).toEqual({ colorless: 1 });
    });

    it("should parse multiple mana of same color", () => {
      const options = parseManaAbility("{T}: Add {G}{G}.");
      expect(options).toHaveLength(1);
      expect(options[0].mana).toEqual({ green: 2 });
    });

    it("should return empty for non-mana abilities", () => {
      const options = parseManaAbility("Flying");
