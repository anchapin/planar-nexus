...
export class ValidationService {
  ...
  public static validateAction(
    ...
    if (player.hasLost) {
      return {
        ...
        message: "You have lost the game and cannot take actions.",
      };
    }
   ...
   */
  private static getMaxLandsPerTurn(
    state: GameState,
    playerId: string,
    modeId?: string,
  ): number {
  ...
  private static hasPriority(state: GameState, action: GameAction): boolean {
    ...
    if (
      ...
      state.turn.currentPhase === "declare_attackers"
    ) {
    ...
    if (
      ...
      state.turn.currentPhase === "declare_blockers"
    ) {
   ...
   */
  private static validatePlayLand(
    state: GameState,
    action: GameAction,
    modeId?: string,
  ): ValidationResult {
    ...
    if (state.turn.activePlayerId !== action.playerId) {
      return {
        ...
        message: "You can only play lands during your turn.",
      };
    }
    ...
    if (
      state.turn.currentPhase !== "precombat_main" &&
      state.turn.currentPhase !== "postcombat_main"
    ) {
      return {
        isValid: false,
        reason: "Wrong phase",
        message: "Lands can only be played during a main phase.",
      };
    }
    ...
    if (state.stack.length > 0) {
      return {
        ...
        message: "You can only play a land when the stack is empty.",
      };
    }

    // Get max lands per turn from game mode config
    const maxLandsPerTurn = this.getMaxLandsPerTurn(
      state,
      action.playerId,
      modeId,
    );
    ...
    if (!isInstant && !hasFlash) {
      ...
      if (state.turn.activePlayerId !== action.playerId) {
        return {
          isValid: false,
          reason: "Not your turn",
          message: "You can only cast this during your turn.",
        };
      }
      ...
      if (
        ...
        return {
          isValid: false,
          reason: "Wrong phase",
          message: "You can only cast this during a main phase.",
        };
      }
      ...
      if (state.stack.length > 0) {
        return {
          isValid: false,
          reason: "Stack not empty",
          message: "You can only cast this when the stack is empty.",
        };
      }
    }
  ...
  private static validateManaCost(
    ...
    if (
      ...
      return {
        isValid: false,
        reason: "Not enough colored mana",
        message: "Not enough colored mana to cast this spell.",
      };
    }
    ...
    if (availableForGeneric < coloredCost.generic) {
      return {
        ...
        message: "Not enough mana to cast this spell.",
      };
    }
   ...
   */
  private static validateDeclareAttackers(
    state: GameState,
    action: GameAction,
  ): ValidationResult {
    if (state.turn.currentPhase !== "declare_attackers") {
      return {
        isValid: false,
        reason: "Not currently in the Declare Attackers step",
      };
    }
    ...
    if (state.turn.activePlayerId !== action.playerId) {
      return {
        isValid: false,
        reason: "Only the active player can declare attackers",
      };
    }
   ...
   */
  private static validateDeclareBlockers(
    state: GameState,
    action: GameAction,
  ): ValidationResult {
    if (state.turn.currentPhase !== "declare_blockers") {
      return {
        isValid: false,
        reason: "Not currently in the Declare Blockers step",
        message: "Not currently in the Declare Blockers step.",
      };
    }
    ...
    if (state.turn.activePlayerId === action.playerId) {
      return {
        isValid: false,
        reason: "The active player cannot declare blockers",
        message: "The active player cannot declare blockers.",
      };
    }
   ...
   */
  private static validateTapUntap(
    state: GameState,
    action: GameAction,
  ): ValidationResult {
    const player = state.players.get(action.playerId);
    ...
    if (card.controllerId !== action.playerId) {
      return {
        isValid: false,
        reason: "Not your card",
        message: "You can only tap/untap cards you control.",
      };
    }
    ...
    if (action.type === "tap_card" && card.isTapped) {
      return {
        ...
        message: "This card is already tapped.",
      };
    }
    ...
    if (action.type === "untap_card" && !card.isTapped) {
      return {
        isValid: false,
        reason: "Already untapped",
        message: "This card is already untapped.",
      };
    }
    ...
    if (action.type === "tap_card" && card.isTapped === false) {
      ...
      if (typeLine.includes("creature") && card.hasSummoningSickness) {
        ...
        return {
          isValid: false,
          reason: "Summoning sickness",
          message: "This creature has summoning sickness.",
        };
      }
    }
