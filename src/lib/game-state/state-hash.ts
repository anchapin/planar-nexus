    ...
    combat: state.combat
      ? {
          ...
          blockerAssignments: Object.fromEntries(
            Array.from(state.combat.blockers.entries()).map(
              ([attackerId, blockers]) => [
                attackerId,
  ...
  for (const playerId of playerIds) {
    ...
    snapshot.players.push({
      ...
      dungeonProgress: player.dungeonProgress
        ? {
            dungeonId: player.dungeonProgress.dungeonId,
            roomId: player.dungeonProgress.roomId ?? "",
            roomIndex: player.dungeonProgress.roomIndex,
          }
        : null,
      completedDungeonIds: [...(player.completedDungeonIds ?? [])].sort(),
    });
  }
...
function serializeStateSnapshot(snapshot: StateSnapshot): string {
  ...
  if (snapshot.turn) {
    parts.push(
      `turn:${snapshot.turn.turnNumber}:${snapshot.turn.activePlayerId}:${snapshot.turn.currentPhase}`,
    );
  }

  for (const player of snapshot.players) {
    parts.push(
      `p:${player.playerId}:${player.life}:${player.poisonCounters}:${player.hasLost}:${player.commanderDamage.join(",")}:${player.dungeonProgress ? `${player.dungeonProgress.dungeonId}:${player.dungeonProgress.roomId}:${player.dungeonProgress.roomIndex}` : "none"}:${player.completedDungeonIds.join(",")}`,
    );
  }
  ...
  if (snapshot.combat) {
    parts.push(
      `combat:${snapshot.combat.inCombatPhase}:${snapshot.combat.attackerIds.join(",")}:${JSON.stringify(snapshot.combat.blockerAssignments)}`,
    );
  }
...
export function analyzeHashDiscrepancy(
  ...
  if (localState.combat && remoteState.combat) {
    ...
    if (
      JSON.stringify(localState.combat.attackers) !==
      JSON.stringify(remoteState.combat.attackers)
    ) {
      discrepancies.push({
        category: "combat",
