...
export const OPTIMAL_MANA_CURVES: Record<DeckFormat, OptimalManaCurve> = {
  commander: {
    ...
    description:
      "Commander curves run higher because longer games and ramp support expensive spells.",
    tips: [
  ...
  standard: {
    ...
    description:
      "Standard decks want a smooth curve peaking at 2 drops with a tapering top end.",
    tips: [
  ...
  modern: {
    ...
    description:
      "Modern is faster than Standard — prioritize efficient 1- and 2-drops.",
    tips: [
 ...
 */
export function normalizeDeckFormat(
 ...
 */
export function getManaCurveTips(
  format: DeckFormat | string = "commander",
): string[] {
  const normalized = normalizeDeckFormat(
    typeof format === "string" ? format : "commander",
  );
...
export function compareToOptimal(
  ...
  const totalGap =
    gaps.reduce((sum, g) => sum + Math.abs(g.difference), 0) +
    (landGap ? Math.abs(landGap.difference) : 0);
...
function analyzeManaCurve(
  ...
  for (const card of cards) {
    if (
      card.type_line &&
      (card.type_line.includes("Land") || card.type_line.includes("land"))
    ) {
      curve[0]++;
      continue;
    }
  ...
  if (highCmcCount > highCmcCap + 2) {
    issues.push(
      `Too many high mana cost cards (6+). For ${format}, aim for ~${highCmcCap} spells at 6+ CMC.`,
    );
    rating -= 2;
  }
  ...
  const earlyTarget =
    profile.buckets[1].target +
    profile.buckets[2].target +
    profile.buckets[3].target;
  if (earlyGame < earlyTarget - 4) {
    issues.push(
      `Not enough early game plays (1-3 mana). For ${format}, aim for ~${earlyTarget} cards at 1-3 CMC.`,
    );
    rating -= 2;
  }
  ...
  if (landCount < profile.lands.min) {
    issues.push(
      `Consider adding more lands (aim for ${profile.lands.min}-${profile.lands.max} in ${format}).`,
    );
    rating -= 1;
  } else if (landCount > profile.lands.max + 3) {
    issues.push(
      "Too many lands. Consider cutting some for more action spells.",
    );
    rating -= 1;
  }
...
function analyzeColors(cards: ScryfallCard[]): ColorDistribution {
  ...
  const colorCount = Object.entries(colors).filter(
    ([k, v]) => k !== "Colorless" && v > 0,
  ).length;
  ...
  const minCount = Math.min(
    ...(colorEntries.filter(([, v]) => v > 0).map(([, v]) => v) || [0]),
  );

  if (colorCount > 1 && maxCount > minCount * 3) {
    issues.push(
      "Color distribution is very uneven. Consider adding more support for weaker colors.",
    );
    rating -= 2;
  }
  ...
  if (colorCount > 3) {
    issues.push(
      "Managing 4+ colors may cause mana issues. Consider a more focused color identity.",
    );
    rating -= 1;
  }
...
function analyzeRemoval(cards: ScryfallCard[]): RemovalAnalysis {
  ...
  if (typeCount < 2 && count > 5) {
    issues.push(
      "Your removal lacks variety. Mix destruction, exile, and counters.",
    );
    rating -= 1;
  }
...
function analyzeRamp(cards: ScryfallCard[]): RampAnalysis {
  ...
  for (const card of cards) {
    ...
    const isRamp =
      ...
      (text.includes("add") &&
        text.includes("mana") &&
        (text.includes("color") || type.includes("artifact"))) ||
      (type.includes("land") &&
        (text.includes("search") || text.includes("put onto the battlefield")));
...
function analyzeSynergies(cards: ScryfallCard[]): SynergyAnalysis {
  ...
  if (
    cardNames.some(
      (n) =>
        n.includes("sorin") || n.includes("vraska") || n.includes("sarkhan"),
    )
  ) {
    if (
      cardNames.some(
        (n) =>
          n.includes("vampire") || n.includes("spirit") || n.includes("zombie"),
      )
    ) {
      pairs.push({
        cards: ["Planeswalker", "Token creatures"],
        description: "Planeswalker + token generation",
      });
    }
  }
  ...
  if (
    cardNames.some((n) => n.includes("counter") || n.includes("proliferate"))
  ) {
    if (
      cardNames.some((n) => n.includes("phyrexian") || n.includes("mikaeus"))
    ) {
      pairs.push({
        cards: ["Counter manipulation", "+1/+1 synergy"],
  ...
  if (
    cardNames.some(
      (n) =>
        n.includes("wheel") || n.includes("notion") || n.includes("rhystic"),
    )
  ) {
    if (
      cardNames.some((n) => n.includes("lobotomy") || n.includes("thought"))
    ) {
      pairs.push({
        cards: ["Wheel effects", "Discard"],
        description: "Wheel + discard synergies",
      });
    }
  }
  ...
  const issues =
    pairs.length === 0
      ? [
          "No obvious synergies detected. Consider adding cards that work well together.",
        ]
      : [];
...
function generateSuggestions(analyses: {
  ...
  if (analyses.rampAnalysis.rating < 5) {
    suggestions.push({
      ...
      title: "Add More Ramp",
      description:
        "Your deck needs more mana acceleration. Add cards like Sol Ring, Arcane Signet, and Cultivate.",
    });
  }
  ...
  if (analyses.removalAnalysis.rating < 5) {
    suggestions.push({
      ...
      description:
        "Your deck needs more answers to threats. Add cards like Swords to Plowshares, Counterspell, or Path to Exile.",
    });
  }
  ...
  if (analyses.colorDistribution.rating < 5) {
    suggestions.push({
      ...
      description:
        "Your color distribution is uneven. Add more dual lands or fix for your weaker colors.",
    });
  }
  ...
  if (analyses.cardTypeDistribution.rating < 5) {
    ...
    if (types.creatures < 10) {
      suggestions.push({
        ...
        description:
          "Your deck needs more creatures for board presence and pressure.",
      });
    }
  }
  ...
  if (analyses.synergyAnalysis.rating < 5) {
    suggestions.push({
      ...
      description:
        "Look for cards that work well together to create more powerful combinations.",
    });
  }
