...
export function summarizeGame(
  ...
  const abilitiesActivated = actions.filter(
  ...
  if (spellsCast > 0)
    gameplayStats.push(
      `cast ${spellsCast} spell${spellsCast === 1 ? "" : "s"}`,
    );
  if (landsPlayed > 0)
    gameplayStats.push(
      `played ${landsPlayed} land${landsPlayed === 1 ? "" : "s"}`,
    );
  if (abilitiesActivated > 0)
    gameplayStats.push(
      `activated ${abilitiesActivated} abilit${abilitiesActivated === 1 ? "y" : "ies"}`,
    );
