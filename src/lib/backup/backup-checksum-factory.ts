/**
 * @fileoverview Browser-only factory for the backup SHA-256 Web Worker
 * (issue #1912).
 *
 * This module is deliberately separate from `backup-checksum-client.ts` because it
...
/**
 * Constructs the backup checksum worker. Returns `null` in environments
 * without the `Worker` global (SSR, Jest/jsdom) or if construction
 * fails, so callers can degrade gracefully instead of crashing.
 */
export function createBackupChecksumWorker(): Worker | null {
  if (typeof Worker === "undefined") return null;

  try {
    const workerUrl = new URL("./backup-checksum.worker.ts", MODULE_URL).href;
    return new Worker(workerUrl, { type: "module" });
  } catch (error) {
    console.error("Failed to create backup checksum worker:", error);
    return null;
  }
}
