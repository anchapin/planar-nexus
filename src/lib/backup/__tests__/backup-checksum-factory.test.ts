/**
 * @fileoverview Tests for the browser-only backup-checksum factory
 * (issue #1912).
 *
 * The factory file (`backup-checksum-factory.ts`) captures
 ...
 *       `backup-checksum-client.ts` was written to handle — a future
...
const FACTORY_PATH = join(__dirname, "..", "backup-checksum-factory.ts");
...
describe("backup-checksum-factory (issue #1912)", () => {
  describe("factory load contract under ts-jest CJS", () => {
    it("throws an ESM-syntax error when require()'d in jsdom (production catch handles it)", () => {
      ...
      try {
        ...
        require("../backup-checksum-factory");
      } catch (error) {
        thrown = error;
      }
  ...
  describe("regression guards for the issue #1912 patterns", () => {
    ...
    it("factory source resolves the worker URL with the static-analysis-friendly shape", () => {
      const code = stripComments(readFileSync(FACTORY_PATH, "utf8"));
      expect(code).toMatch(
        /new\s+URL\s*\(\s*['"]\.\/backup-checksum\.worker\.ts['"][\s\S]*?MODULE_URL\s*,?\s*\)\.href/,
      );
    });
  });
});
