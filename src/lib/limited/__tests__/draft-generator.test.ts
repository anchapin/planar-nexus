...
describe("DRFT-02: 3 packs of 14 cards each", () => {
  ...
  it("should generate total of 42 cards (3 packs × 14 cards)", async () => {
    const session = await createDraftSession("M21", "Core Set 2021");
    const totalCards = session.packs.reduce(
...
// ============================================================================
// Issue #1443 — AiDifficulty migration
// ============================================================================

describe("normalizeAiDifficulty (issue #1443)", () => {
  it("passes canonical values through unchanged", () => {
    expect(normalizeAiDifficulty("easy")).toBe("easy");
    expect(normalizeAiDifficulty("medium")).toBe("medium");
    expect(normalizeAiDifficulty("hard")).toBe("hard");
    expect(normalizeAiDifficulty("expert")).toBe("expert");
  });
  ...
  it("maps legacy archival names onto their canonical tier", () => {
    expect(normalizeAiDifficulty("beginner")).toBe("easy");
    expect(normalizeAiDifficulty("normal")).toBe("medium");
    expect(normalizeAiDifficulty("master")).toBe("expert");
  });

  it("defaults unknown / null / undefined / empty values to medium", () => {
    expect(normalizeAiDifficulty(undefined)).toBe("medium");
    expect(normalizeAiDifficulty(null)).toBe("medium");
    expect(normalizeAiDifficulty("")).toBe("medium");
    expect(normalizeAiDifficulty("garbage")).toBe("medium");
    expect(normalizeAiDifficulty(42)).toBe("medium");
  });

  it("persisted sessions created with a legacy difficulty still resolve", () => {
    // Simulates an older IndexedDB row with 'master' in aiNeighbor.difficulty.
    // The normalize helper must collapse it without throwing so the session
    // can be reopened across builds (#1443 acceptance criteria).
    expect(normalizeAiDifficulty("master" as unknown)).toBe("expert");
    expect(normalizeAiDifficulty("beginner" as unknown)).toBe("easy");
  });
});
