...
export function useTournamentEvents(
  ...
  const [myRegistrations, setMyRegistrations] = useLocalStorage<
    { eventId: string }[]
  ...
  const updateEvent = useCallback(
    (eventId: string, updates: Partial<TournamentEvent>) => {
  ...
  const unregisterFromEvent = useCallback(
    (eventId: string, playerId: PlayerId) => {
      setActiveEvents((prev) =>
        prev.map((e) => {
          if (e.id !== eventId) return e;
  ...
  // Open registration
  const openEventRegistration = useCallback(
    (eventId: string) => {
      setActiveEvents((prev) =>
  ...
  const finishTournament = useCallback(
    (eventId: string, standings: EventStandings[]) => {
      setActiveEvents((prev) => {
