...
jest.mock("@/components/ui/collapsible", () => ({
  Collapsible: ({ children, className, ...props }: any) => (
    <div data-testid="collapsible" className={className} {...props}>
      {children}
    </div>
  ),
...
import {
  KeyCards,
  identifyKeyCards,
} from "@/app/(app)/deck-coach/_components/key-cards";
import {
  ExportButton,
  formatReportAsText,
} from "@/app/(app)/deck-coach/_components/export-button";
...
describe("ArchetypeBadge", () => {
  ...
  it("should render with custom className", () => {
    ...
    expect(
      screen.getByText("Test").closest(".custom-class"),
    ).toBeInTheDocument();
  });
...
describe("ExportButton", () => {
  const mockReport = {
    ...
    missingSynergies: [],
    keyCards: [{ name: "Lightning Bolt", reason: "Core card", count: 4 }],
