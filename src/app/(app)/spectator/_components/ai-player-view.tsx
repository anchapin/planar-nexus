...
export function AIPlayerView({
  ...
  const availableMana = Object.entries(player.manaPool)
    ...
    .map(([color, count]) => ({
      ...
      colorClass:
        MANA_COLORS[color as keyof typeof MANA_COLORS] || "bg-gray-300",
    }));
    ...
    <Card className={`p-4 mb-4 ${borderColor} ${bgColor} transition-colors`}>
      ...
      <CardContent className="space-y-3">
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            <Heart
              className={`h-5 w-5 ${player.life <= 5 ? "text-red-500 animate-pulse" : "text-red-500"}`}
            />
            ...
            <div className="flex flex-wrap gap-1">
              {availableMana.map(({ color, count, symbol, colorClass }) => (
                <div key={color} className="flex items-center gap-1">
                  ...
                  {/*
                    Issue #1602 (WCAG 1.4.1 / 4.1.2): the mana type must not be
                    conveyed by the colored emoji/background alone. The badge
                    exposes an accessible name that includes the color word.
                  */}
                  <span
                    className={`inline-flex items-center justify-center w-6 h-6 rounded-full text-xs font-bold ${colorClass}`}
                    title={color}
                    role="img"
                    aria-label={`${count} ${color} mana`}
                  >
                    {symbol}
                  </span>
                </div>
        ...
        <div className="grid grid-cols-3 gap-2">
          ...
          <div className="space-y-1">
            ...
            {/* Issue #1602 (WCAG 4.1.2): the progressbar needs an accessible name. */}
            <Progress
              value={handProgress}
              className="h-1"
          ...
          <div className="space-y-1">
            ...
            <div className="text-xs text-muted-foreground">untapped/total</div>
          </div>
          ...
          <div className="space-y-1">
            ...
            <div className="text-lg font-semibold">{librarySize}</div>
            <div className="text-xs text-muted-foreground">cards left</div>
          </div>
        </div>
      </CardContent>
    </Card>
