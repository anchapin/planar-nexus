...
import {
  ...
  type GameSpeed,
  SPEED_CONFIG,
} from "./_components/spectator-controls";
...
export default function SpectatorPage() {
  ...
  const commentaryHistoryRef = useRef<CommentaryHistory>(
    new CommentaryHistory(50),
  );
  ...
  const initializeGame = useCallback(() => {
    try {
      ...
      updatedState = loadDeckForPlayer(
        updatedState,
        players[1].id,
        player2Deck,
      );
      ...
      commentaryHistoryRef.current.add(
        initialCommentary.generateMessage(
          "⚔️ AI vs AI Spectator Mode initialized",
        ),
      );
      commentaryHistoryRef.current.add(
        initialCommentary.generateMessage(
  ...
  const runTurnWithCommentary = useCallback(
    async (state: GameState) => {
      ...
      try {
        ...
        const result = await runAITurn(state, currentPlayerId, {
          difficulty: "medium",
        ...
        addCommentary(
          commentary_gen.generateMessage(
            `Error during ${currentPlayer.name}'s turn`,
          ),
        );
      }
    },
  ...
  useEffect(() => {
    ...
    if (gameState.winners.length > 0) {
      ...
      addCommentary(
        commentary_gen.generateWin(
          gameState.winners[0],
          gameState.endReason || "Victory",
        ),
      );
      setIsPlaying(false);
      return;
        ...
        <div className="space-y-4">
            ...
            <div className="space-y-2 text-sm">
              ...
              <div className="flex justify-between">
                ...
                <span className="capitalize">
                  {gameState?.turn.currentPhase?.replace(/_/g, " ") || "-"}
                </span>
              </div>
              <div className="flex justify-between">
          ...
          <div className="p-4 border rounded-lg bg-card">
            ...
            <ul className="text-sm text-muted-foreground space-y-2 list-disc list-inside">
              <li>
                Click <strong>Start</strong> to begin the match
              </li>
              ...
              <li>
                Use <strong>Export</strong> to save game history
              </li>
              <li>
                Click <strong>Restart</strong> for a new game
              </li>
            </ul>
          </div>
