...
import type { DeckCard, SavedDeck } from "@/lib/card-database";
    ...
    switch (cardType) {
      ...
      case "enchantment":
        card = createCard({
          ...options,
          name: options?.name
            ? `${options.name} Enchantment ${i + 1}`
            : `Test Enchantment ${i + 1}`,
 ...
 */
export function createCommanderDeck(
