...
- **AI client wrappers** live in `src/lib/ai-client.ts` (`getDeckReview`, `generateOpponent`) — plain client-side functions around the AI flows, **not** server actions. Both `getDeckReview` and `generateOpponent` have heuristic fallbacks when AI providers are unavailable or misconfigured. Deck-persistence types (`DeckCard`, `SavedDeck`) live in `src/lib/card-database.ts`, which also re-exports the engine-owned card shape (`ScryfallCard`/`MinimalCard` from `src/lib/game-state/types/card-data.ts`, #1724). (The old misnamed `src/app/actions.ts` was removed in #1592.)
- **Rules engine** lives in `src/lib/game-state/` — a large MTG rules implementation (layer-system, replacement-effects, trigger-system, state-based-actions, spell-casting, combat, mana, …). This is the correctness-critical core and the only place mutation testing runs. Its **only public API is the barrel** (`src/lib/game-state/index.ts`); eslint errors on any `@/lib/game-state/*` deep import outside the engine (#1710).
- **AI**: multi-provider via Vercel AI SDK (`@ai-sdk/openai|anthropic|google|react`) with a unified proxy route at `src/app/api/ai-proxy/`; Genkit flows in `src/ai/flows/` (deck-coach-review, opponent-generation). Provider keys in `.env` (`OPENAI_* / ANTHROPIC_* / GOOGLE_* / ZAI_*`). AI routes perform a fast-fail health check (5s timeout) before attempting calls; unhealthy providers are skipped in the failover chain. Both `getDeckReview` and `generateOpponent` have heuristic fallbacks that need **no** API key.
- **Persistence**: IndexedDB-first — decision record, inventory table, and conventions in `docs/PERSISTENCE_ARCHITECTURE.md` (#1722). Tier-1 user data lives in `PlanarNexusStorage` via the raw-IDB wrapper `src/lib/indexeddb-storage.ts` (versioned migrations, currently v4). Dexie is sanctioned only for `src/lib/db/local-intelligence-db.ts` and `src/lib/limited/pool-storage.ts`. Quota is owned by `src/lib/storage-quota.ts`; backup by `indexeddb-storage.ts` → `backup-compression.ts` → `use-storage-backup.ts`. Tests use `fake-indexeddb`.
