...
- **AI client wrappers** live in `src/lib/ai-client.ts` (`getDeckReview`, `generateOpponent`) — plain client-side functions around the AI flows, **not** server actions. Deck-persistence types (`DeckCard`, `SavedDeck`) live in `src/lib/card-database.ts`, which also re-exports the engine-owned card shape (`ScryfallCard`/`MinimalCard` from `src/lib/game-state/types/card-data.ts`, #1724). (The old misnamed `src/app/actions.ts` was removed in #1592.)
...
- **Persistence**: IndexedDB-first, 10 databases across two sanctioned stacks — decision record, inventory table, and conventions in `docs/PERSISTENCE_ARCHITECTURE.md` (#1722). Tier-1 user data lives in `PlanarNexusStorage` via the raw-IDB wrapper `src/lib/indexeddb-storage.ts` (versioned migrations); Dexie is sanctioned only for `src/lib/db/local-intelligence-db.ts` and `src/lib/limited/pool-storage.ts` (`dexie-react-hooks` is declared but unused). Quota is owned by `src/lib/storage-quota.ts`; backup by `indexeddb-storage.ts` → `backup-compression.ts` → `use-storage-backup.ts`. Tests use `fake-indexeddb`.
